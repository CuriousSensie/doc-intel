...
/**
 * specs/01-architecture.md §Event bridge's primary path: infra/scripts/notify-pomocnik.sh POSTs
...
export async function POST(request: Request) {
  try {
    ...
    const signature = request.headers.get("x-pomocnik-signature");
    const timestamp = request.headers.get("x-pomocnik-timestamp");
    ...
    const valid = verifyPaperlessWebhookSignature(
      ...
      timestamp,
      signature,
      requireEnv("POMOCNIK_WEBHOOK_SECRET")
    );
    if (!valid) {
    ...
    if (paperlessDocumentId == null) {
      // notify-pomocnik.sh sends `paperless_document_id: null` when Paperless didn't pass
    ...
    // signature itself — notify-pomocnik.sh has no event id of its own to give us, and the
    ...
    // document Pomočnik hasn't synced yet) is not an error: the reconciliation sweep
