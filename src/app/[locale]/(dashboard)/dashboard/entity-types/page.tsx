...
export default async function EntityTypesAdminPage() {
  ...
  const t = await getTranslations("entityTypes");
          ...
          <Link href={`/dashboard/entity-types/${type.id}`} key={type.id}>
            <Card className="transition-colors hover:bg-panel-strong/40">
              <CardHeader className="flex-row items-center justify-between gap-3 space-y-0">
                ...
                {type.is_system ? <Badge variant="muted">{t("admin.system")}</Badge> : null}
              </CardHeader>
              <CardContent>
                <p className="text-sm text-muted">{t("admin.keyLabel", { key: type.key })}</p>
              </CardContent>
            </Card>
          </Link>
        ))}
