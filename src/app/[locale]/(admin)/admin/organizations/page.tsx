  ...
  return (
    <div className="grid gap-5">
      <section className="rounded-lg border border-border bg-panel p-6 shadow-sm">
        <h1 className="text-3xl font-black">{t("organizationsPage.title")}</h1>
        <form action="/admin/organizations" className="mt-5 flex gap-3">
          <TextField
            defaultValue={params.q ?? ""}
            label={t("organizationsPage.searchLabel")}
            name="q"
          />
          <Button className="self-end" type="submit" variant="outline">
            {t("organizationsPage.search")}
          </Button>
        </form>
        <div className="mt-4">
                ...
                <div className="flex flex-wrap gap-2">
                  ...
                  >
                    ...
                    <Button size="sm" type="submit" variant="outline">
                      {organization.suspended_at
                        ? t("organizationsPage.unsuspend")
                        : t("organizationsPage.suspend")}
                    </Button>
                  </form>
                  ...
                  organization.provisioning_status === "provisioning_failed" ? (
                    <form action={reprovisionOrganizationAction}>
                      <input name="organizationId" type="hidden" value={organization.id} />
                      <Button size="sm" type="submit" variant="outline">
                        {t("organizationsPage.reprovision")}
                      </Button>
                    </form>
                  ) : null}
                ...
                <div className="mt-4 flex flex-wrap items-end gap-4 border-t border-border pt-4">
                  {isFeatureEnabled("credits") ? (
                    <form action={adjustCreditsAction} className="flex items-end gap-2">
                      <input name="organizationId" type="hidden" value={organization.id} />
                      <TextField
                        label={t("organizationsPage.creditAdjustmentLabel")}
                        name="amount"
                        placeholder={t("organizationsPage.creditAdjustmentPlaceholder")}
                        type="number"
                      />
                      <Button size="sm" type="submit" variant="outline">
                        {t("organizationsPage.adjustCredits")}
                      </Button>
                    </form>
                  ) : null}
                  {isFeatureEnabled("billing") ? (
                    <div className="flex gap-2">
                      <form action={toggleSubscriptionPlatformStatusAction}>
                        ...
                        <Button size="sm" type="submit" variant="outline">
                          {t("organizationsPage.disableSubscription")}
                        </Button>
                      </form>
                      <form action={toggleSubscriptionPlatformStatusAction}>
                        <input name="organizationId" type="hidden" value={organization.id} />
                        ...
                        <Button size="sm" type="submit" variant="outline">
                          {t("organizationsPage.enableSubscription")}
                        </Button>
                      </form>
                    </div>
                  ) : null}
