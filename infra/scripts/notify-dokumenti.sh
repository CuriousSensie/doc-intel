#!/bin/sh
# Dokumenti post-consume bridge. Paperless 3.x passes metadata as env vars only (no positional
...
DOKUMENTI_URL="${DOKUMENTI_INTERNAL_URL:-http://web:3000}/api/internal/paperless/document-consumed"
SECRET="${DOKUMENTI_WEBHOOK_SECRET:?DOKUMENTI_WEBHOOK_SECRET must be set in the Paperless container's env}"
TIMESTAMP="$(date +%s)"
...
curl --silent --show-error --max-time 5 \
  --request POST "${DOKUMENTI_URL}" \
  --header "Content-Type: application/json" \
  --header "X-Dokumenti-Signature: ${SIGNATURE}" \
  --header "X-Dokumenti-Timestamp: ${TIMESTAMP}" \
  --data "${BODY}" \
  || true
