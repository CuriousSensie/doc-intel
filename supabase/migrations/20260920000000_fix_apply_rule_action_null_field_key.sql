-- Real bug found via live browser testing: fieldKeyForAction() in rules.dispatcher.ts
-- deliberately returns null for add_tag/remove_tag (tags are additive, not a single-value field
-- slot — no field-claim conflict to track), but apply_rule_action() unconditionally inserted
-- p_field_key into field_provenance.field_key, a NOT NULL column. Every add_tag/remove_tag rule
-- action raised 23502 ("null value in column field_key violates not-null constraint") after the
-- Paperless-side tag write had already succeeded, so the whole dispatchRuleActions() call threw,
-- rule_runs.actions_applied was never recorded, and rule_backfills.applied_count stayed 0 no
-- matter how many documents matched. Fix: only write field_provenance when there's an actual
-- field key to attribute.
