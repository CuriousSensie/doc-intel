...
-- validation failure. rules.dispatcher.ts's resolveEntityRef() now checks organization_id for
-- the entity_ref-by-id case (found and fixed this session — "identifier"/"name" refs were
-- already scoped, "id" was not), but apply_rule_action() itself trusted its source_id/target_id
-- arguments with no ownership check at all — defense in depth, matching
-- connections.service.ts#assertBelongsToOrg()'s reasoning for a client-supplied connection
-- target. A caller bug or future code path (not just the current dispatcher) must not be able to
-- create a cross-tenant connection through this function.
